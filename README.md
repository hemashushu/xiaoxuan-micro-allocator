...
- [原理](#-原理)
  ...
  - [_Heap_ 的结构](#-_heap_-的结构)
    ...
    - [_Data area_ 的结构](#-_data-area_-的结构)
      ...
      - [标记位](#-标记位)
  - [_Free block link list_ 的工作过程](#-_free-block-link-list_-的工作过程)
...
- `magic number`：[uint32, uint32]，用于表示 _XiaoYu Allocator_ 的幻数，值为 HEX `78 79 61 6c 6c 6f 63 01`，对应字符串 `xyalloc`，最后的 `0x01` 表示版本号。`magic number` 用于标识 _heap_ 是否已经初始化；
- `data area pos`：uint32，_data area_ 的开始地址；
- `index page pos`：uint32，_index page_ 的开始地址；
- `first free block pos`：uint32，第一个 _allocated block_ 的地址；
...
下面的图演示了一个包含有 3 个 _allocated block_ 和 2 个 _free block_ 的 _data area_：
...
- `==` 表示 _allocated block_；
- `//` 表示 _free block_。

_可变大小分配器_ 会 **按照位置的顺序** 将所有 _free block_ 串联起来，形成一个显式的双向链表，称为 _free block linked list_。分配器使用该链表管理空闲的内存。
...
```
| block size field [31:2]  |  [1:1]  |  [0:0]  |
| block size               | status  | type    |
```
...
### _Free block link list_ 的工作过程

当分配器初始化之后，_free block link list_ 只包含一个 _free block_，该 _free block_ 占据了整个 _data area_。
...
这种情况需要合并后一个 _free block_。具体需要更新的有：
...
_固定大小分配器_（_fix size allocator_）是 _XiaoYu Allocator_ 的一个子系统，它运行于 _可变大小分配器_ 的基础之上。用于快速分配具有固定大小且容量较小（小于等于 128 bytes）的内存。
...
固定大小分配器由 1 个 _索引页_（_index page_） 和 32 个 _数据页_（_data page_） 组成。_数据页_ 里面是一个单向链表。索引页包含有 32 个 uint32 整数（称为 _head item offset_），每一个整数指向一个单向链表的表头。

_索引页_ 和 _数据页_ 均被包装在 _allocated block_ 里，如下图所示：

```
|-----------------|  |---------------------|  |----------------------|
| allocated block |  | allocated block     |  | allocated block      |
||---------------||  ||-------------------||  ||--------------------||
||   index page  ||  || data page 1       ||  || data page 2        ||
||---------------||  ||-------------------||  ||--------------------||
|-----------------|  |---------------------|  |----------------------|
```
...
当一个数据页的所有 _空闲项_ 都被分配之后，分配器会新建一个同类的 _数据页_，并且将新的 _数据页_ 的第一个 _free item_ 连接到 _free item linked list_ 的末尾。
...
- `next free item offset`：下一个 _空闲项_ 位置相对 _index page_ 的偏移值；
- `class idx`：5 bits，当前项的 _类_ 的索引值，值的范围是从 0 到 31；
...
`padding` 字段实际上是 _空闲项_ 的 `next free item offset` 字段，因为 _内存项_ 被分配出去之后，原先 `next free item offset` 字段的值此已不再具有任何意义，该字段的值既可以清零，也可以保持不变。

注意，无论是 _allocated block_ 还是 _已分配项_，分配器返回给应用程序的都是其中的 `data` 字段的位置（内存指针）。当 _allocated block_ 或者 _已分配项_ 被释放时，比如应用程序调用了函数 `void free(int addr)`，通过参数传递给分配器的其实是 `data` 字段的位置。分配器需要根据这个地址往后 4 bytes 读取一个 uint32 整数，这个整数的低端 2 bits 就是 _标记位_。通过 _标记位_ 的 `type` flag 就可以区分该内存片段是 _allocated block_ 还是 _已分配项_，知道内存片段的类型之后再由相应的分配程序来执行具体的释放过程。
